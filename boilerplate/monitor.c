...
{
    struct monitored_entry *curr, *tmp;
    long rss;

    mutex_lock(&monitor_lock);
    list_for_each_entry_safe(curr, tmp, &monitored_list, list) {
        ...
        if (rss > curr->hard_limit_bytes) {
            ...
            list_del(&curr->list);
            kfree(curr);
        } else if (rss > curr->soft_limit_bytes && !curr->soft_limit_warned) {
            log_soft_limit_event(curr->container_id, curr->pid, curr->soft_limit_bytes, rss);
            curr->soft_limit_warned = true;
        }
    }
